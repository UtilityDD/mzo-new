      ...
      <div className="px-4 pt-6 space-y-6 max-w-lg mx-auto md:max-w-4xl w-full">
        ...
        <div className="space-y-4">
          <div className="bg-white p-6 rounded-[32px] android-shadow border border-gray-100">
             <h4 className="text-sm font-bold text-gray-800 mb-4 uppercase tracking-widest">Category Distribution</h4>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
             <div className="bg-white p-6 rounded-[32px] android-shadow border border-gray-100">
                <h4 className="text-sm font-bold text-gray-800 mb-4 uppercase tracking-widest">Connection Status</h4>
             ...
             <div className="bg-white p-6 rounded-[32px] android-shadow border border-gray-100">
                <h4 className="text-sm font-bold text-gray-800 mb-4 uppercase tracking-widest">Phase Breakdown</h4>
                <div className="h-48 w-full">
                   <ResponsiveContainer width="100%" height="100%">
        ...
        <div className="bg-white rounded-[32px] android-shadow border border-gray-100 overflow-hidden">
           <div className="p-5 border-b border-gray-50 flex justify-between items-center">
              <h4 className="text-xs font-bold text-gray-400 uppercase tracking-widest">Detail View</h4>
              <span className="text-[10px] font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full">{data.length} Groups</span>
           </div>
           <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                 <thead className="bg-gray-50/50">
                    <tr>
                       <th className="px-5 py-4 font-bold text-gray-400">Class</th>
                       <th className="px-5 py-4 font-bold text-gray-400">Status</th>
                       <th className="px-5 py-4 font-bold text-gray-400 text-right">Count</th>
                       <th className="px-5 py-4 font-bold text-gray-400 text-right">Load</th>
                    </tr>
                 </thead>
                    ...
                    {loading ? (
                       [...Array(3)].map((_, i) => (
                          <tr key={i} className="animate-pulse">
                             <td colSpan={4} className="px-5 py-6"><div className="h-4 bg-gray-100 rounded-lg w-full"></div></td>
                          </tr>
                       ))
                    ) : (
                       data.slice(0, 15).map((row, i) => (
                          <tr key={i} className="hover:bg-gray-50 transition-colors">
                             <td className="px-5 py-5">
                                <p className="font-bold text-gray-900">{row.BASE_CLASS}</p>
                                <p className="text-[10px] text-gray-400">{row.CATEGORY}</p>
                             </td>
                             <td className="px-5 py-5">
                                <span className={`text-[10px] font-bold px-2 py-1 rounded-full ${row.CONN_STAT === 'LIVE' ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'}`}>
                                   {row.CONN_STAT}
                                </span>
                             </td>
                             <td className="px-5 py-5 text-right font-black text-gray-700">{row.COUNT.toLocaleString()}</td>
                             <td className="px-5 py-5 text-right font-bold text-blue-600">{row.LOAD.toLocaleString()}</td>
                          </tr>
                       ))
                    )}
                 </tbody>
