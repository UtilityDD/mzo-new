         ...
         <div className="px-4 pt-6 space-y-6 max-w-lg mx-auto md:max-w-4xl w-full">
            ...
            <div className="flex flex-col gap-4">
               ...
               <div className="flex gap-2 overflow-x-auto pb-2 hide-scrollbar">
                  {['DelayRange', 'PoleNonPole', 'APPLICANT_TYPE'].map(fType => (
                     <select
                        key={fType}
                        className="bg-white px-5 py-2.5 rounded-2xl text-[11px] font-bold text-gray-600 border border-gray-100 android-shadow min-w-[130px] active:scale-95 transition-all outline-none focus:ring-2 focus:ring-blue-500/20"
                        onChange={(e) => {
            ...
            <div className="space-y-4">
               <div className="flex justify-between items-center px-2">
                  <h4 className="text-[10px] font-extrabold text-gray-400 uppercase tracking-widest">Application Queue</h4>
               ...
               {loading && data.length === 0 ? (
                  ...
                  data.slice(0, 50).map((row, idx) => (
                     <div
                        ...
                        className={`bg-white rounded-[32px] android-shadow border transition-all active:scale-[0.99] overflow-hidden group fade-in ${expandedId === row.APPL_NO ? 'border-blue-200 ring-4 ring-blue-50' : 'border-gray-100'}`}
                        style={{ animationDelay: `${idx * 0.03}s` }}
                     >
                        <div className="p-5 flex justify-between items-start">
                           <div className="space-y-2 flex-1 pr-4">
                              <div className="flex flex-wrap items-center gap-2">
                                 <span className="text-[9px] font-extrabold text-blue-600 bg-blue-50 px-2.5 py-1 rounded-lg uppercase tracking-wider border border-blue-100">{row.APPL_NO}</span>
                                 {row.IS_DUARE_SARKAR === 'Y' && <span className="text-[9px] font-extrabold bg-orange-100 text-orange-600 px-2.5 py-1 rounded-lg uppercase border border-orange-200">Duare Sarkar</span>}
                              </div>
                              <h3 className="text-lg font-bold text-gray-900 leading-tight group-hover:text-blue-600 transition-colors uppercase tracking-tight">{row.NAME}</h3>
                              <p className="text-xs text-gray-400 font-semibold flex items-center gap-1.5">
                                 ...
                                 <span className="uppercase text-[10px]">{row.ccc_code}</span>
                              </p>
                           </div>
                           <div className="text-right">
                              <div className={`text-sm font-black px-4 py-2 rounded-2xl inline-block shadow-sm ${row.DelayInSC > 15 ? 'bg-rose-50 text-rose-600 border border-rose-100' : 'bg-emerald-50 text-emerald-600 border border-emerald-100'}`}>
                                 {row.DelayInSC}d
                              </div>
                              <p className="text-[9px] text-gray-400 mt-2 font-extrabold uppercase tracking-widest leading-none">Delay</p>
                           </div>
                        </div>
                        ...
                        {expandedId === row.APPL_NO && (
                           <div className="px-6 pb-6 pt-2 border-t border-gray-50 bg-gray-50/50 space-y-6 animate-in fade-in slide-in-from-top-2">
                              <div className="grid grid-cols-2 gap-x-6 gap-y-5 pt-4">
                                 ...
                                 <div>
                                    <p className="text-[10px] text-gray-400 uppercase font-extrabold tracking-widest mb-1.5 opacity-60">Applied Date</p>
                                    <p className="text-sm font-bold text-gray-800 flex items-center gap-2">
                                 ...
                                 <div>
                                    <p className="text-[10px] text-gray-400 uppercase font-extrabold tracking-widest mb-1.5 opacity-60">Connection Type</p>
                                    <p className="text-[10px] font-black text-gray-700 bg-white px-2 py-1 rounded-md border border-gray-100 inline-block">{row.CONN_TYPE}</p>
                                 </div>
                                 <div>
                                    <p className="text-[10px] text-gray-400 uppercase font-extrabold tracking-widest mb-1.5 opacity-60">SCN Progress</p>
                                    <p className={`text-[10px] font-black px-2 py-1 rounded-md inline-block uppercase tracking-wider ${row.SCN_STATUS === 'PENDING' ? 'bg-rose-50 text-rose-600 border border-rose-100' : 'bg-blue-50 text-blue-600 border border-blue-100'}`}>{row.SCN_STATUS}</p>
                                 </div>
                              </div>
                              ...
                              <div className="flex gap-3 pt-2">
                                 <a href={`tel:${row.PHONE_NO}`} className="flex-1 py-4 bg-blue-600 text-white rounded-2xl text-[11px] font-bold text-center active:scale-95 transition-all premium-shadow flex items-center justify-center gap-2">
                                 ...
                                 <button className="flex-1 py-4 bg-white text-gray-700 rounded-2xl text-[11px] font-bold active:scale-95 transition-all border border-gray-200 hover:bg-gray-50 flex items-center justify-center gap-2">
                                    <i className="fa-solid fa-pen-to-square"></i> Log Action
                                 </button>
                              </div>
                           </div>
                        )}
